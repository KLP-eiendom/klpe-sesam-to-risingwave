...
**Sink in failed state (e.g. PubSub PermissionDenied, topic not found)**
- `ALTER SINK ... RESUME` is not supported in RisingWave
- Fix: recreate the sink via `./deploy.sh <env> --select <sink_name> --full-refresh`

## KLP / KdiRisingWave Project

### New PubSub sink checklist

Every new `google_pubsub` sink in `dbt/models/sinks/` requires matching OpenTofu infrastructure in **all three environments** (`dev`, `test`, `prod`) under `terraform/<env>/risingwave_pubsub.tf`.

**Required resources per topic (4 blocks):**
1. `google_pubsub_topic` — main topic (`<name>-${var.environment}`)
2. `google_pubsub_topic` — dead-letter topic (`<name>-dead-letter-${var.environment}`)
3. `google_pubsub_subscription` — worker subscription with dead-letter policy and retry
4. `google_pubsub_subscription` — dead-letter subscription with retry

Use an existing block (e.g. `bygg-leko` or `kredittvurdering-superoffice`) as the template. The `risingwave-sa` service account gets `roles/pubsub.publisher` project-wide, so no per-topic IAM binding is needed.

```hcl
resource "google_pubsub_topic" "my-topic" {
  name = "my-topic-${var.environment}"
}

resource "google_pubsub_topic" "my-topic-dead-letter" {
  name = "my-topic-dead-letter-${var.environment}"
}

resource "google_pubsub_subscription" "my-topic-worker" {
  name  = "my-topic-worker-${var.environment}"
  topic = google_pubsub_topic.my-topic.name

  ack_deadline_seconds = 120
  dead_letter_policy {
    dead_letter_topic     = google_pubsub_topic.my-topic-dead-letter.id
...
resource "google_pubsub_subscription" "my-topic-deadletter" {
  name  = "my-topic-deadletter-${var.environment}"
  topic = google_pubsub_topic.my-topic-dead-letter.name
...
**To verify coverage:** grep all `pubsub.topic` values from `dbt/models/sinks/*.sql` and confirm each has a matching `google_pubsub_topic` resource in `risingwave_pubsub.tf`.

### New PubSub sink — topic naming

All PubSub sink files use `target.name` in the default to automatically resolve the correct environment suffix — **no Vault configuration needed** for topic names:

```sql
pubsub.topic = '{{ env_var("GCP_PUBSUB_TOPIC_MY_TOPIC", "my-topic-" ~ target.name) }}',
```

This produces `my-topic-dev`, `my-topic-test`, or `my-topic-prod` automatically. An env var override in Vault (`GCP_PUBSUB_TOPIC_MY_TOPIC`) is still supported if a non-standard name is needed, but is not required.

**If RisingWave fails with `pubsub.topics.create` PermissionDenied:** the topic doesn't exist in GCP yet — run `tofu apply` in the relevant environment first, then redeploy the sink:
```bash
./deploy.sh dev --select snk_my_sink --full-refresh
```
